...
public final class CommandFacade {
    ...
    private ObjectNode handleAssignTicket(final JsonNode cmdNode, final User user) {
        final String username = cmdNode.get("username").asText();
        final String timestamp = cmdNode.get("timestamp").asText();
        ...
        final Milestone ms = (msName == null) ? null : state.getMilestone(msName);

        final Developer dev = (user instanceof Developer) ? (Developer) user : null;
        ...
        if (dev != null && requiresMidOrSenior(t)) {
            SeniorityLevel curSen = dev.getSeniorityLevel();
            if (curSen == SeniorityLevel.JUNIOR) {
                return OutputBuilder.start("assignTicket", username, timestamp)
                        .error("Developer " + username + " cannot assign ticket " + ticketId
    ...
    private static boolean requiresMidOrSenior(final Ticket t) {
        return t.getBusinessPriority() == BusinessPriority.HIGH
                || t.getBusinessPriority() == BusinessPriority.CRITICAL;
    }

    private static List<ExpertiseArea> allowedExpertise(final String ticketExp) {
        List<ExpertiseArea> s = new ArrayList<>();
        if (ticketExp == null || ticketExp.isEmpty()) return s;

        s.add(ExpertiseArea.valueOf(ticketExp));
        s.add(ExpertiseArea.FULLSTACK);
        return s;
    }
    ...
    private ObjectNode handleChangeStatus(final JsonNode cmdNode, final User user) {
        ...
        if (to == TicketStatus.CLOSED) {
             String msName = state.getMilestoneNameForTicket(ticketId);
             if (msName != null) {
                 Milestone ms = state.getMilestone(msName);
                 if (ms != null && !ms.isActive(state)) {
                     for (String blockedMsName : ms.getBlockingFor()) {
                         Milestone blockedMs = state.getMilestone(blockedMsName);
                         if (blockedMs != null && !blockedMs.isBlocked(state)) {
                             String msg = "Milestone " + blockedMsName + " is now unblocked as ticket " + ticketId + " has been CLOSED.";
                             for (String dev : blockedMs.getAssignedDevs()) {
                                 state.pushNotification(dev, msg);
                             }
                         }
                     }
    ...
    private ObjectNode handleViewTicketHistory(final JsonNode cmdNode, final User user) {
        ...
        ArrayNode arr = out.putArray("ticketHistory");
        for (Ticket t : mine) {
            arr.add(t.toHistoryJson());
        }
        return out;
    }
    private ObjectNode handleUndoChangeStatus(
            final JsonNode cmdNode, final User user) {
    ...
    private ObjectNode handleSearch(
            final JsonNode cmdNode,
            final User user) {

        final String username = cmdNode.get("username").asText();
        final String timestamp = cmdNode.get("timestamp").asText();

        final JsonNode filters = cmdNode.get("filters");
        final String searchType = filters.get("searchType").asText();
    ...
    private ObjectNode handleSearchDeveloper(
        ...
        List<Developer> matched = new ArrayList<>();
        for (User u : state.users.values()) {
            if (!(u instanceof Developer)) continue;
            Developer d = (Developer) u;
            
            if (requester.getRole() == Role.MANAGER) {
                Manager m = (Manager) requester;
                if (!m.getSubordinates().contains(d.getUsername())) {
                    continue;
                }
            }
        ...
        matched.sort(Comparator.comparing(User::getUsername));
    ...
    private ObjectNode handleSearchTicket(
            final String username,
            final String timestamp,
            final JsonNode filters) {
        ...
        List<Ticket> base = new ArrayList<>(state.getTickets());
        ...
        visible.sort(Comparator.comparingInt(Ticket::getId));
    ...
    private boolean canAssignForSearch(final Ticket t, final Developer dev) {
        String msName = state.getMilestoneNameForTicket(t.getId());
        ...
        if (requiresMidOrSenior(t) && dev.getSeniorityLevel() == SeniorityLevel.JUNIOR) {
            return false;
        }
    ...
    private static int daysUntilDueInclusive(final String nowIso, final String dueIso) {
        LocalDate now = LocalDate.parse(nowIso);
    ...
    private void generateMilestoneNotifications(final String nowIso) {
        ...
        for (Milestone ms : state.getAllMilestones()) {
            ...
            LocalDate due = LocalDate.parse(ms.getDueDate());
    ...
    private ObjectNode handleViewNotifications(
            final JsonNode cmdNode,
            final User user) {
        ...
        List<String> notes = state.consumeNotifications(username);
        ...
        ArrayNode arr = out.putArray("notifications");
        for (String s : notes) {
            arr.add(s);
        }
        return out;
    }
    private ObjectNode handleGenerateCustomerImpactReport(
            final JsonNode cmdNode,
            final User user) {
        ...
        ObjectNode tbt = report.putObject("ticketsByType");
        for (Map.Entry<String, Integer> e : byType.entrySet()) tbt.put(e.getKey(), e.getValue());

        ObjectNode tbp = report.putObject("ticketsByPriority");
        for (Map.Entry<String, Integer> e : byPriority.entrySet()) tbp.put(e.getKey(), e.getValue());

        ObjectNode ci = report.putObject("customerImpactByType");
    ...
    private static double featureImpactScore(final Ticket t) {
        double bv;
    ...
    private static double uiFeedbackImpactScore(final Ticket t) {
        ...
        switch (t.getBusinessValue()) {
            ...
            default:   bv = 10.0; break;
        }

        int us = (t.getUsabilityScore() == null) ? 5 : t.getUsabilityScore();
        double usabilityFactor = (11.0 - us) / 10.0;
    ...
    private ObjectNode handleGenerateTicketRiskReport(
            final JsonNode cmdNode,
            final User user) {
        ...
        String bugRisk = computeRiskLabelForType(TicketType.BUG, considered);
        String frRisk = computeRiskLabelForType(TicketType.FEATURE_REQUEST, considered);
        String uiRisk = computeRiskLabelForType(TicketType.UI_FEEDBACK, considered);
        ...
        ObjectNode rbt = report.putObject("riskByType");
        rbt.put("BUG", bugRisk);
    ...
    private static String computeRiskLabelForType(final TicketType type,
                                                  final List<Ticket> considered) {
        int count = 0;
        double sum = 0.0;
    ...
    private ObjectNode handleGenerateResolutionEfficiencyReport(
            final JsonNode cmdNode,
            final User user) {
        ...
        double bugEff = efficiencyForType(considered, TicketType.BUG);
        double frEff  = efficiencyForType(considered, TicketType.FEATURE_REQUEST);
        double uiEff  = efficiencyForType(considered, TicketType.UI_FEEDBACK);
        ...
        ObjectNode eff = report.putObject("efficiencyByType");
        eff.put("BUG", bugEff);
    ...
    private static double efficiencyForType(final List<Ticket> tickets,
        ...
        for (Ticket t : tickets) {
            ...
            sumExpected += getPriorityWeight(t.getBusinessPriority());
            sumActual += (daysBetweenIso(t.getCreatedAt(), lastResolvedAt(t)) + 1);
        }
    ...
    private static double getPriorityWeight(final BusinessPriority p) {
        switch (p) {
            case CRITICAL: return 6.0;
            case HIGH:     return 5.9;
    ...
    private static String lastResolvedAt(final Ticket t) {
        String last = "";
        for (TicketAction a : t.getActions()) {
            if ("STATUS_CHANGED".equals(a.getAction()) && 
                ("RESOLVED".equals(a.getTo()) || "CLOSED".equals(a.getTo()))) {
                last = a.getTimestamp();
            }
        }
        return last.isEmpty() ? t.getCreatedAt() : last; 
    }

    private static int daysBetweenIso(final String startIso, final String endIso) {
        LocalDate a = LocalDate.parse(startIso);
        LocalDate b = LocalDate.parse(endIso);
        long diff = ChronoUnit.DAYS.between(a, b);
        return diff < 0 ? 0 : (int) diff;
    }

    private ObjectNode handleAppStabilityReport(
            final JsonNode cmdNode,
            final User user) {
        ...
        String bugRisk = stabilityRiskLabel(TicketType.BUG, open);
        String frRisk  = stabilityRiskLabel(TicketType.FEATURE_REQUEST, open);
        String uiRisk  = stabilityRiskLabel(TicketType.UI_FEEDBACK, open);
        ...
        ObjectNode obt = report.putObject("openTicketsByType");
        for (Map.Entry<String, Integer> e : byType.entrySet()) obt.put(e.getKey(), e.getValue());

        ObjectNode obp = report.putObject("openTicketsByPriority");
        for (Map.Entry<String, Integer> e : byPriority.entrySet()) obp.put(e.getKey(), e.getValue());
        ...
        ObjectNode ibt = report.putObject("impactByType");
    ...
    private static String stabilityRiskLabel(final TicketType type,
    ...
    private static double priorityRiskWeight(final BusinessPriority p) {
        switch (p) {
    ...
    private static double uiFeedbackStabilityImpactScore(final Ticket t) {
        int rank;
    ...
    private ObjectNode handleGeneratePerformanceReport(final JsonNode cmdNode, final User user) {
        ...
        List<String> subordinateUsernames = manager.getSubordinates();
